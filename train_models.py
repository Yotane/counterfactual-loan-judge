...
TARGET_COL = "ever_seriously_delinquent"
...
def outcome_objective(trial: optuna.Trial, X_train: pd.DataFrame, y_train: pd.Series, X_val: pd.DataFrame, y_val: pd.Series) -> float:
    # optuna search space for outcome model hyperparameters
    params = {
...
def train_outcome_model(df: pd.DataFrame, feature_cols: list, n_trials: int = 50, seed: int = 1) -> tuple[lgb.LGBMClassifier, dict]:
    # optimize lightgbm with optuna, train final model, compute metrics
    X = df[feature_cols]
    y = df[TARGET_COL]
    ...
    study.optimize(lambda trial: outcome_objective(trial, X_train, y_train, X_val, y_val), n_trials=n_trials, show_progress_bar=False)
    
    logger.info(f"Best Optuna AUC-ROC: {study.best_value:.4f}")
...
def generate_shap_explanations(model: lgb.LGBMClassifier, df: pd.DataFrame, feature_cols: list, n_samples: int = 5000) -> pd.DataFrame:
    ...
    shap_df["base_value"] = explainer.expected_value[1] if isinstance(explainer.expected_value, list) else explainer.expected_value
    return shap_df
...
def prepare_econml_datasets(df: pd.DataFrame, feature_cols: list) -> dict:
    ...
    # add propensity scores as control for doubly robust estimation
    if "propensity_score" in df.columns:
        W["propensity_score"] = df["propensity_score"]
    
    logger.info(f"EconML ready: X={X.shape}, T={T.shape}, Y={Y.shape}, W={W.shape}")
    return {"X": X, "T": T, "Y": Y, "W": W, "feature_names": feature_cols}

def sql_bi_export(df: pd.DataFrame, output_path: Path):
    # duckdb query for power bi/tableau export: risk segmentation and treatment effect proxies
    con = duckdb.connect()
    ...
    SELECT 
        ...
        SUM(ever_seriously_delinquent) as delinquent_count,
        ROUND(100.0 * SUM(ever_seriously_delinquent) / COUNT(*), 2) as delinquency_rate_pct,
